...
import { RealL1Read } from "@hyper-evm-lib/test/utils/RealL1Read.sol";
import { CoreView } from "@hyper-evm-lib/test/simulation/hyper-core/CoreView.sol";

contract TestCoreExecution is CoreView {
    ...
    function executeTokenTransfer(
        ...
        if (_accounts[from].activated) {
            _accounts[from].spot[token] += toWei(
                value,
                _tokens[token].evmExtraWeiDecimals
            );
        } else {
            _latentSpotBalance[from][token] += toWei(
                value,
                _tokens[token].evmExtraWeiDecimals
            );
        }
    ...
    function executeNativeTransfer(
        ...
        if (_accounts[from].activated) {
            ...
            _latentSpotBalance[from][HYPE_TOKEN_INDEX] += (value / 1e10)
                .toUint64();
        }
    }
    ...
    function executePerpLimitOrder(
        ...
        PrecompileLib.Position memory position = _accounts[sender].positions[
            perpIndex
        ];
    ...
    function _executePerpLong(
        ...
        if (szi >= 0) {
            ...
            _accounts[sender].positions[perpIndex].entryNtl +=
                uint64(action.sz) * uint64(markPx);
            ...
            _accounts[sender].perpBalance -=
                (uint64(action.sz) * uint64(markPx)) / leverage;

            _accounts[sender].margin[perpIndex] +=
                (uint64(action.sz) * uint64(markPx)) / leverage;
        } else {
            if (newSzi <= 0) {
                uint64 avgEntryPrice = _accounts[sender]
                    .positions[perpIndex]
                    .entryNtl / uint64(-szi);
                int64 pnl = int64(action.sz) *
                    (int64(avgEntryPrice) - int64(_markPx));

                uint64 closedMargin = ((uint64(action.sz) *
                    _accounts[sender].positions[perpIndex].entryNtl) /
                    uint64(-szi)) / leverage;
                ...
                _accounts[sender].perpBalance = pnl > 0
                    ? _accounts[sender].perpBalance + uint64(pnl)
                    : _accounts[sender].perpBalance - uint64(-pnl);
                uint64 oldMargin = _accounts[sender]
                    .positions[perpIndex]
                    .entryNtl / leverage;
        ...
        if (szi <= 0) {
            ...
            if (newSzi >= 0) {
                uint64 avgEntryPrice = _accounts[sender]
                    ...
                    .entryNtl / uint64(szi);
                int64 pnl = int64(action.sz) *
                    (int64(_markPx) - int64(avgEntryPrice));
                uint64 closedMargin = ((uint64(action.sz) *
                    _accounts[sender].positions[perpIndex].entryNtl) /
                    uint64(szi)) / leverage;
                _accounts[sender].perpBalance += closedMargin;
    ...
    function executeSpotLimitOrder(
        ...
        public
        initAccountWithSpotMarket(
            sender,
            uint32(HLConversions.assetToSpotId(action.asset))
        )
    {
        PrecompileLib.SpotInfo memory spotInfo = RealL1Read.spotInfo(
            uint32(HLConversions.assetToSpotId(action.asset))
        );

        PrecompileLib.TokenInfo memory baseToken = _tokens[spotInfo.tokens[0]];

        uint64 spotPx = readSpotPx(
            uint32(HLConversions.assetToSpotId(action.asset))
        );
        ...
        if (isActionExecutable(action, spotPx)) {
            if (action.isBuy) {
                ...
                uint64 amountOut = action.sz *
                    (10 ** (baseToken.weiDecimals - baseToken.szDecimals))
                        .toUint64();
            ...
            _pendingOrders.push(
                PendingOrder({ sender: sender, action: action })
            );
        }
    }
        ...
        initAccountWithToken(action.destination, action.token)
    {
        ...
        if (_accounts[action.destination].activated == false) {
            ...
            _accounts[action.destination].spot[action.token] +=
                _latentSpotBalance[sender][action.token] + action._wei;
        ...
        if (action.destination != systemAddress) {
            ...
            if (action.token == HYPE_TOKEN_INDEX) {
                ...
                (bool success, ) = address(sender).call{
            ...
            transferAmount = fromWei(
                action._wei,
                _tokens[action.token].evmExtraWeiDecimals
            );
            deal(
                evmContract,
                systemAddress,
                IERC20(evmContract).balanceOf(systemAddress) + transferAmount
            );
        ...
        whenActivated(sender)
    {
        if (action.toPerp) {
            if (
                fromPerp(action.ntl) <= _accounts[sender].spot[USDC_TOKEN_INDEX]
            ) {
                _accounts[sender].perpBalance += action.ntl;
                _accounts[sender].spot[USDC_TOKEN_INDEX] -= fromPerp(
                    action.ntl
                );
            }
        } else {
    ...
    function executeVaultTransfer(
        ...
        _accounts[sender]
            .vaultEquity[action.vault]
            .equity = readUserVaultEquity(sender, action.vault).equity;
        _userVaultMultiplier[sender][action.vault] = _vaultMultiplier[
            action.vault
        ];

        if (action.isDeposit) {
            if (action.usd <= _accounts[sender].perpBalance) {
                _accounts[sender].vaultEquity[action.vault].equity += action
                    .usd;
                _accounts[sender]
                    .vaultEquity[action.vault]
                    .lockedUntilTimestamp = uint64(
            ...
            PrecompileLib.UserVaultEquity storage userVaultEquity = _accounts[
                sender
            ].vaultEquity[action.vault];
            ...
            if (
                action.usd <= userVaultEquity.equity &&
                userVaultEquity.lockedUntilTimestamp / 1000 <= block.timestamp
            ) {
    ...
    function executeStakingWithdraw(
        ...
        whenActivated(sender)
    {
        PrecompileLib.DelegatorSummary memory summary = readDelegatorSummary(
            sender
        );
        ...
        whenActivated(sender)
    {
        ...
        PrecompileLib.Delegation storage delegation = _accounts[sender]
            .delegations[action.validator];
        ...
        if (action.isUndelegate) {
            if (
                action._wei <= delegation.amount &&
                block.timestamp * 1000 > delegation.lockedUntilTimestamp
            ) {
                _accounts[sender].staking += action._wei;
                delegation.amount -= action._wei;

                if (delegation.amount == 0) {
                    _accounts[sender].delegatedValidators.remove(
                        action.validator
                    );
                }
            } else {
                revert(
                    "Insufficient delegation amount OR Delegation is locked"
                );
            }
        } else {
            if (action._wei <= _accounts[sender].staking) {
                _accounts[sender].staking -= action._wei;
                _accounts[sender].delegations[action.validator].amount += action
                    ._wei;

                _accounts[sender]
                    .delegations[action.validator]
                    .lockedUntilTimestamp = ((block.timestamp + 86400) * 1000)
                    .toUint64();
            } else {
    ...
    function isActionExecutable(
        LimitOrderAction memory action,
        uint64 px
    ) internal pure returns (bool) {
        bool executable = action.isBuy
            ? action.limitPx >= px
            : action.limitPx <= px;
        return executable;
    }
    ...
    function processPendingOrders() public {
        for (uint256 i = _pendingOrders.length; i > 0; i--) {
            ...
            if (isActionExecutable(order.action, spotPx)) {
                ...
                _pendingOrders[i - 1] = _pendingOrders[
                    _pendingOrders.length - 1
                ];
                _pendingOrders.pop();
            }
        }
    ...
    function isLiquidatable(address user) public returns (bool) {
        ...
        for (uint256 i = len; i > 0; i--) {
            ...
            PrecompileLib.Position memory pos = _accounts[user].positions[
                perpIndex
            ];
            if (pos.szi != 0) {
        ...
        int64 equity = int64(_accounts[user].perpBalance) +
            int64(totalLocked) +
            totalUPnL;
    ...
    // Added for testing - force position creation
    function forcePosition(
        ...
        _accounts[account].positions[perpIndex] = PrecompileLib.Position({
            szi: szi,
            entryNtl: entryNtl,
            isolatedRawUsd: 0, // Not in isolated margin
            leverage: 1, // Default leverage
            isIsolated: false
        });
        if (szi != 0) {
            _userPerpPositions[account].add(perpIndex);
        } else {
            _userPerpPositions[account].remove(perpIndex);
        }
    }
}
