...
contract DebugAllocationScript is Script {
    ...
    function run() external {
        ...
        PrecompileLib.SpotBalance memory hypeBalance = PrecompileLib.spotBalance(
            address(manager),
            hypeTokenId
        );
        ...
        // Check perp position
        PrecompileLib.Position memory perpPosPrecompile = PrecompileLib.position(
            address(manager),
        ...
        PrecompileLib.AccountMarginSummary memory marginSummary = PrecompileLib.accountMarginSummary(
            0,
            address(manager)
        );
